...
Path =  "C:/CyBorG/IIT Kharagpur/Sudoku_Cancer/Based_onJhForrest_Comment/Generation of Sudoku Instances/3) Creating base files to solve using NEW method/Modelling LOTSIZE variables with INTEGER variables/Sudoku_Filtered_2.xlsx"
...
for sheet in sheets:
    ...
    for r in ROWS:
        for c in COLS:
            choices[r,c]=prob.addVar(vtype = GRB.INTEGER, name = "x_%d_%d"% (r,c))

    # The values taken by each variable are summed intelligently
    sum=0
    all_Values=""
    for val in ROWS:
        actual_powered_up_value=int(pow(2,val-1)-1)
        sum+=actual_powered_up_value
        all_Values+=","+str(actual_powered_up_value)
    print(all_Values)
    print(sum)

    # We do not define an objective function since none is needed

    # The row, column and box constraints are added
    ...
    #print("Printing the given values")
    for key in given_Values:    
        #print(key,"  :  ",given_Values[key])
    ...
    LotFileName ='Filtered_Sudoku_Size'+str(Sudoku_size)+'.lot'
    with open(LotFileName, 'w') as a:
        print("")
